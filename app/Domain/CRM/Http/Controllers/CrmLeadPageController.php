...
class CrmLeadPageController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $leads = Lead::query()->where('company_id', $companyId)->with(['stage', 'owner', 'tags'])
            ->when($filters['search'] ?? null, fn ($q, $v) => $q->where(fn ($q) => $q->where('name', 'like', "%{$v}%")->orWhere('email', 'like', "%{$v}%")->orWhere('phone', 'like', "%{$v}%")))
        ...
        $sources = Lead::query()->where('company_id', $companyId)->whereNotNull('source')->distinct()->orderBy('source')->pluck('source');
    ...
    public function show(Lead $lead): View
    {
        ...
        return view('crm.leads.show', [
            ...
            'availableTags' => CrmTag::query()->where('company_id', $this->context->companyId())->orderBy('name')->get(),
        ]);
    }
    ...
    public function kanban(): View
    {
        ...
        $pipelines = CrmPipeline::query()->where('company_id', $this->context->companyId())->where('status', 'active')
            ->with(['stages' => fn ($q) => $q->where('status', 'active')->with(['leads' => fn ($q) => $q->where('company_id', $this->context->companyId())->with(['owner', 'tags'])->latest()])->orderBy('position')])
