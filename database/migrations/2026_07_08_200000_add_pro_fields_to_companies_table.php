...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('companies', function (Blueprint $table) {
            if (!Schema::hasColumn('companies', 'slug')) {
                $table->string('slug')->nullable()->unique()->after('trade_name');
            }

            if (!Schema::hasColumn('companies', 'website')) {
                $table->string('website')->nullable()->after('owner');
            }

            if (!Schema::hasColumn('companies', 'zip_code')) {
                $table->string('zip_code', 20)->nullable()->after('website');
            }
    ...
    public function down(): void
    {
        Schema::table('companies', function (Blueprint $table) {
            $columns = [
                'slug', 'website', 'zip_code', 'address', 'number', 'complement',
                'district', 'city', 'state', 'logo', 'notes', 'created_by', 'deleted_at'
            ];
